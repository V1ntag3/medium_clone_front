...
function ProfileInfo({ imageProfile, about, followers, followings }) {
    ...
    return (
        ...
        <Container style={{ background: '#FFC017', height: 'auto', borderBottom: '1px solid black', display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
            <div style={{ width: '100%', maxWidth: 700 }}> <img alt='logout' onClick={() => logout()} style={{ width: 25, marginTop: 10, float: 'right', marginRight: 15, cursor: 'pointer' }} src={Logout} /> <Link to="/articleCreate"><span style={{ float: 'right', padding: 9, background: 'black', color: 'white', borderRadius: 20, marginTop: 5, marginRight: 10 }}>Add Article</span></Link></div>
